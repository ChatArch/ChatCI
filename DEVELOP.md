...
- Attach `chatstyle.add_tree_option()` to the top-level Click CLI with the public `chatci` root name.
- Keep parameter signatures in `--tree`; use `--tree-brief` for command nodes and descriptions without signatures.
